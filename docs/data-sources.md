...
When an approved inspection feed becomes available, a nearby map centers on the active DC Pulse search location and respects its radius. The Map defaults to closures, follow-up-required inspections, and inspections with at least one Priority or Priority Foundation violation. An explicit filter may reveal all supported outcomes. The filter must explain that hidden results are routine or lower-severity records, not missing data. Each marker must open establishment, inspection date/outcome, notable violations, and authoritative attribution.

A fixture-backed, transport-independent version 1 feed adapter now enforces stable schema/version identity, explicit DC Health attribution, HTTPS source/report links, valid in-DC coordinates, nonnegative violation counts, freshness, and a local kill switch before normalizing any record. The query boundary uses the active search center and radius; its default attention filter includes closures, follow-up-required inspections, and either Priority class, while an explicit all-results filter includes routine records. This is pre-exposure infrastructure only: no supported production feed has been identified and no HTML is consumed. See [Restaurant inspection feed contract](restaurant-inspection-feed-contract.md).

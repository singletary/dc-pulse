...
- [ ] **P0 — Complete cached-first Map loading with per-source reconciliation.** Extend the existing bounded multi-context cache so compatible launch/Home results seed Map immediately, in-flight requests are shared, sources refresh independently, reliable update fields are used where supported, and cached records survive partial failures.

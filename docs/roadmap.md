...
- Define one stable, privacy-safe DC fallback center for denied, restricted, unavailable, or not-yet-determined location access. Load useful default Map results without presenting them as the person's current location, and show a non-blocking banner explaining that DC Pulse is more useful in DC with location access enabled. Include a clear route to system settings plus existing ward and DC-address browsing options; do not repeatedly interrupt launch with a ward chooser.
- For an authorized location just outside DC, derive the nearest usable search center on or slightly inside the District boundary, load that nearby DC area, and explain the adjustment in a dismissible banner. If the person is meaningfully farther away, use the same documented fallback center as the no-location path and explain that DC Pulse is showing a representative DC area instead. Select and document the near/far threshold only after testing realistic travel and approximate-location cases.
- Treat permission state, raw device location, effective DC search center, and user-facing location label as separate state. Never display **Using your current location** when results are boundary-adjusted or fallback-based, and never persist an inferred out-of-DC position as Home without explicit confirmation.
- Add deterministic geometry and state tests for each authorization state, approximate accuracy, locations on every side of the DC boundary, near-versus-far routing, banner dismissal and recovery actions, relaunch, and a later transition to a valid in-DC location. Add a physical-iPhone pass for denial, Settings re-enable, and travel into or out of DC.
...
- Add a native **About DC Pulse** and **Acknowledgments** surface before public App Store release, reachable from an existing secondary destination such as Places or a settings/overflow menu rather than adding another primary tab. Include the installed version/build, app website, public GitHub repository, support and privacy pages, MIT license, DC public-data attribution, and the independent/non-government-service disclaimer.
- Keep open-source software notices distinct from open-data attribution: the app source is MIT licensed, while each DC dataset retains its publisher's terms and attribution. The current app has no third-party package dependencies; if that changes, generate dependency-specific notices from verified license metadata instead of maintaining an error-prone hand-written list.
- Keep essential About and license text readable offline, open external destinations through standard secure system links, never expose a private developer or App Review contact, and add accessibility plus URL-validation coverage.

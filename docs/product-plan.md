...
The current ranked execution plan is maintained in [roadmap.md](roadmap.md). Version 1.0 (build 5) has completed its initial internal physical-iPhone pass and is submitted for external TestFlight beta review. It stabilizes initial loading and progressive Map population while preserving build 4's map completeness, coherent summary totals, Requests refresh, photo-source selection, and notification improvements. Opportunistic Background App Refresh remains the first planned notification delivery model; server-backed APNs are deferred until product use justifies them.
...
- **Civic actions:** provide an official DC 311 submission/check-status destination and a native X compose handoff with a pre-filled status-update request, falling back to the web composer when X is not installed. The user reviews and sends the post in X; DC Pulse never posts on their behalf. The draft keeps its continuation action reachable and offers the official DC311 app plus an explicit website fallback; direct submission remains contract-gated.
...
- **Watching:** individual requests and permits can be saved on-device and reconciled when fresh matching data is loaded. An opt-in auto-watch rule saves new 311 requests and permits within 0.1 or 0.25 mile of home when those records are refreshed. People can keep resolved explicit watches active for 7, 30, or 90 days—or indefinitely—while auto-watched records use a seven-day window before moving to a visible, restorable archive that no longer participates in routine checks.
- **Notifications:** a persistent in-app inbox records watched-item status changes and newly auto-watched records near Home, with unread state and direct Item Details navigation. It works without system notification permission. Local system alerts remain explicitly opt-in, provide separate choices for watched status changes and new items near Home, and fire only after an app refresh detects matching activity. Their lock-screen copy omits street addresses. Enabling auto-watch prompts for permission when the system has not asked yet. Launch, manual refresh, and throttled foreground refresh all reconcile watches. Reliable background or remote delivery still requires Background Modes or a small server-side polling and push service.

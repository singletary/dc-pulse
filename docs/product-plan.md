...
- **Notifications:** a persistent in-app inbox records watched-item status changes and newly auto-watched records within 0.1 mile of Home, with unread state and direct Item Details navigation. It works without system notification permission. Local system alerts remain explicitly opt-in, provide separate choices for watched status changes and new items within that 0.1-mile boundary, and fire only after an app refresh detects matching activity. Auto-watch may retain items from the wider selected radius without creating a new-near-Home notification outside 0.1 mile. Their lock-screen copy omits street addresses. Enabling auto-watch prompts for permission when the system has not asked yet. Launch, manual refresh, and throttled foreground refresh all reconcile watches. Reliable background or remote delivery still requires Background Modes or a small server-side polling and push service.

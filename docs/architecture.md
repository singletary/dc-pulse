...
`PulseDataStore` loads the first live 311 repository for the shared Pulse, Activity, and Map experiences and exposes idle/loading/content/empty/error state. Downtown DC is the initial architectural place while Core Location permission UX remains a later slice. SwiftData will later back Places and preferences. Caching and refresh stay behind repository protocols.

...
`Features/Report311` owns the photo-first request draft. `ReportPhotoAnalyzing` isolates image understanding from SwiftUI and currently uses Apple's on-device Vision classifier. Photo metadata is not read; the existing location service supplies a current DC location when permission is available, and manual address entry remains the fallback. The view requires confirmation and attempts to hand the draft to the official portal; current TestFlight defects in keyboard dismissal and portal rendering are release blockers, not acceptable steady-state behavior. The corrected flow must keep its primary action visible with the keyboard present, support deliberate dismissal, launch a physically verified official destination, and fail with a useful fallback. A future submission client must be a separately injected protocol backed by a documented DC interface.

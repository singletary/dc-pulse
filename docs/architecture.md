...
`WatchedPulseItem` records whether a watch was explicitly chosen or added by the Home auto-watch rule. A transition into the adapters' normalized resolved state starts a 30-day explicit-watch or 7-day automatic-watch grace period; reopening cancels it. After the grace period, a successful refresh moves the record into a visible archive and excludes it from routine refresh batches without deleting its snapshot or notification history. Manual archive and restore actions use the same reversible state. Newly added optional persistence fields keep the existing SwiftData store eligible for lightweight migration.

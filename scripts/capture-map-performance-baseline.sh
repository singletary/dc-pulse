...
if [[ $# -lt 3 || $# -gt 5 ]]; then
    echo "usage: $0 <simulator-udid> <DCPulse.app> <output.csv> [run-pairs=5] [timeout-seconds=75]" >&2
    exit 64
fi
...
echo "cache,run,interactive_s,first_markers_s,close_in_s,bounded_s,coverage_session_s,final_items,outcome,dc311_total_s,building_permits_total_s,ddot_permits_total_s,failed_source_requests,dc311_failures,building_permits_failures,ddot_permits_failures" > "$output_csv"
...
capture_run() {
    ...
    xcrun simctl spawn "$device_udid" log show \
        --last 2m --info --debug --signpost --style ndjson \
        --predicate "processIdentifier == $process_id AND subsystem == \"$bundle_id\" AND category == \"MapPerformance\" AND (signpostName == \"Map Presentation Started\" OR signpostName == \"Map Interactive\" OR signpostName == \"First Map Markers\" OR signpostName == \"Close-in Coverage Complete\" OR signpostName == \"Selected Radius Coverage Complete\" OR signpostName == \"Bounded Map Coverage Complete\" OR signpostName == \"Map Coverage Session\" OR signpostName == \"Map Source Request\")" \
        > "$log_file" 2>/dev/null

    xcrun swift "$summarizer" "$cache_state" "$run_number" < "$log_file" >> "$output_csv"

...
struct ArcGISQuery: Equatable, Sendable {
    ...
    struct Statistic: Codable, Equatable, Sendable {
        ...
        let onStatisticField: String
        let outStatisticFieldName: String
    }
    ...
    func url(for layerURL: URL) throws -> URL {
        ...
        if !outputFields.isEmpty { items.append(URLQueryItem(name: "outFields", value: outputFields.joined(separator: ","))) }
        if returnCountOnly { items.append(URLQueryItem(name: "returnCountOnly", value: "true")) }
        if let point {
            items += [
        ...
        if !statistics.isEmpty {
            guard let data = try? JSONEncoder().encode(statistics),
                  let value = String(data: data, encoding: .utf8) else { throw ArcGISClientError.invalidRequest }
            items.append(URLQueryItem(name: "outStatistics", value: value))
        }
        if !groupByFieldsForStatistics.isEmpty {
            items.append(URLQueryItem(name: "groupByFieldsForStatistics", value: groupByFieldsForStatistics.joined(separator: ",")))
        }
